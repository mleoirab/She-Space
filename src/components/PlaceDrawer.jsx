...
export default function PlaceDrawer({ place, presenceByPlace, setPresenceByPlace, ratingsByPlace }) {
  ...
  const ratingsSummary = summary
    ? `Support score=${summary.community.toFixed(1)} (n=${summary.count}).`
    : "No ratings yet.";
  ...
  return (
    <div className="card">
      ...
      <button className="primaryBtn" style={{ marginTop: 10 }}
      ...
      <p className="muted small">Summarizes recent comments into supportive guidance (no identities stored).</p>
      ...
      {aiResult && (
        <div className="aiBox">
          ...
          {Array.isArray(aiResult.tags) && aiResult.tags.length > 0 && <p><b>Tags:</b> {aiResult.tags.join(", ")}</p>}
          {aiResult.recommendation && <p><b>Recommendation:</b> {aiResult.recommendation}</p>}
          {aiResult.raw && (<pre className="pre">{aiResult.raw}</pre>)}
        </div>
      )}
    </div>
  );
